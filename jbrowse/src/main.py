...
def handle_new_mapping(message):
    ...
    try:
        ...
        subprocess.check_output(
            args=['jbrowse', 'add-track', storage_bam, '--name', message.mapping_name, '--category', 'mapping', '--load', 'symlink'],
            cwd=jbrowse_assembly_path
        )
        return True
    except subprocess.CalledProcessError:
...
def handle_new_annotation(message):
    ...
    try:
        ...
        subprocess.check_output(
            args=['jbrowse', 'add-track', '{}.gz'.format(storage_gff), '--name', message.annotation_name, '--category', 'annotation', '--load', 'symlink'],
            cwd=jbrowse_assembly_path
        )
        return True
    except subprocess.CalledProcessError:
