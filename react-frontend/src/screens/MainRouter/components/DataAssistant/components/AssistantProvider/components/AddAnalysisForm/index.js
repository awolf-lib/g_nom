...
const AddAnalysisForm = (props) => {
  ...
  const handleSubmitImport = async () => {
    ...
    if (!newAnalysisName) {
      handleNewNotification({
        label: "Error",
        message: "Missing new analysis name!",
        type: "error",
      });
      return;
    }
    if (!selectedPath.length) {
      handleNewNotification({
        label: "Error",
        message: "Missing path to anaylsis file!",
        type: "error",
      });
      return;
    }
  ...
  return (
    <div className="animate-grow-y">
      <div className="flex items-center">
        <div className="w-64 font-semibold">New analysis name:</div>
        <Input
...
AddAnalysisForm.defaultProps = {};

AddAnalysisForm.propTypes = {};
