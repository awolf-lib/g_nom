...
-- -----------------------------------------------------
-- Table `gnom_db`.`analysesTaxaminer`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `gnom_db`.`analysesTaxaminer` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `analysisID` BIGINT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `analysisID_UNIQUE` (`analysisID` ASC) VISIBLE,
  CONSTRAINT `TaxaminerIDAnalysisID`
    FOREIGN KEY (`analysisID`)
    REFERENCES `gnom_db`.`analyses` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `gnom_db`.`runningTasks` (
  ...
  `targetTaxon` BIGINT UNSIGNED NULL,
  `targetAssembly` BIGINT UNSIGNED NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `targetTaxon_UNIQUE` (`targetTaxon` ASC) VISIBLE,
  UNIQUE INDEX `targetAssembly_UNIQUE` (`targetAssembly` ASC) VISIBLE)
ENGINE = InnoDB;
